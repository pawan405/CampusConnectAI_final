            ...
            {["discussion", "doubts", "groups"].map((tab) => (
              <button
                ...
                className={`text-[10px] font-black uppercase tracking-widest transition-all ${
                  activeTab === tab ? "text-cyan-400" : "text-white/40 hover:text-white"
                }`}
              >
                {tab}
              </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
              ...
              {activeTab === "discussion" && (
                ...
                >
                      ...
                      <div className="flex gap-4">
                        ...
                        <div className="flex-1 space-y-4">
                          <Textarea 
                            ...
                            className="bg-transparent border-none text-lg resize-none p-0 focus-visible:ring-0 min-h-[100px]"
                          />
                  ...
                  <div className="space-y-6">
                    {posts.map((post) => (
                      <Card key={post.id} className="bg-black/40 border-white/5 rounded-3xl hover:bg-white/[0.02] transition-colors group">
                        <CardContent className="p-8 space-y-6">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                <p className="text-[10px] text-white/20 uppercase font-bold tracking-widest">{post.time}</p>
                              </div>
                            </div>
                            <Button variant="ghost" size="icon" className="text-white/20 opacity-0 group-hover:opacity-100">
                          ...
                          <div className="flex items-center gap-6 pt-6 border-t border-white/5">
                            ...
                            <button className="flex items-center gap-2 text-white/40 hover:text-emerald-400 transition-colors ml-auto">
              ...
              {activeTab === "doubts" && (
                ...
                >
                  ...
                  <div className="space-y-4">
                    {doubts.map((doubt) => (
                      <Card key={doubt.id} className="bg-black/40 border-white/5 rounded-[32px] hover:border-purple-500/30 transition-all cursor-pointer group">
                        <CardContent className="p-8">
                          <div className="flex items-start justify-between mb-4">
                            ...
                            <span className="text-[10px] font-bold text-white/20 uppercase tracking-widest">{doubt.time}</span>
                          </div>
                          <p className="text-white/60 text-sm leading-relaxed mb-6">{doubt.question}</p>
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              <div className="w-6 h-6 rounded-full bg-white/5 flex items-center justify-center text-[10px] font-bold">
                                {doubt.user[0]}
                              </div>
                              <span className="text-xs font-bold text-white/40">{doubt.user}</span>
                            </div>
              ...
              {activeTab === "groups" && (
                ...
                >
                  {colleges.map((college, i) => (
                    <Card key={i} className="bg-black/40 border-white/5 rounded-[40px] overflow-hidden hover:border-cyan-500/30 transition-all group">
                      <CardHeader className="p-8 border-b border-white/5">
                        ...
                        <CardDescription className="text-white/40 font-bold uppercase tracking-widest text-[10px] pt-2">{college.members} Neurons Active</CardDescription>
                      </CardHeader>
                      <CardContent className="p-8">
          ...
          <div className="lg:col-span-4 space-y-8">
              ...
              <CardContent className="p-8 space-y-4">
                {["#QuantumComputing", "#AIAesthetics", "#NeuralRoadmaps", "#CyberSecurity"].map((tag, i) => (
                  <div key={i} className="flex items-center justify-between group cursor-pointer">
                    ...
                    <Badge className="bg-white/5 border-none text-[9px] font-black">{12 + i * 5}k</Badge>
                  </div>
                ))}
              </CardContent>
            </Card>
            ...
            <Card className="bg-gradient-to-br from-purple-500/10 to-cyan-500/10 border-white/10 rounded-[32px] p-8 space-y-6">
              ...
              <Button variant="outline" className="w-full rounded-xl border-white/10 text-[10px] font-black uppercase tracking-widest">
                Access Mentors
              </Button>
            </Card>
          </div>
