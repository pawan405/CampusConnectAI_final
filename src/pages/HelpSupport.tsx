          ...
          <div className="lg:col-span-7 space-y-12">
            <section className="space-y-8">
              ...
              <Accordion type="single" collapsible className="space-y-4">
                {faqs.map((faq, i) => (
                  <AccordionItem key={i} value={`item-${i}`} className="border-white/5 bg-white/[0.02] rounded-3xl px-8 overflow-hidden">
                    <AccordionTrigger className="hover:no-underline py-6">
                      <span className="text-left font-bold text-lg text-white/80 group-hover:text-white">{faq.q}</span>
                    </AccordionTrigger>
                    <AccordionContent className="text-white/40 text-sm leading-relaxed pb-8">
                      {faq.a}
                    </AccordionContent>
                  </AccordionItem>
                ))}
              ...
              <CardHeader className="p-10 border-b border-white/5">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-white/40 text-xs uppercase tracking-widest font-bold">Contact our support neurons</CardDescription>
                  </div>
                </div>
              </CardHeader>
                ...
                <form onSubmit={handleContactSubmit} className="space-y-6">
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-white/20 uppercase tracking-widest pl-1">Identity</label>
                      <Input 
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-white/20 uppercase tracking-widest pl-1">Communication Link</label>
                      <Input 
                        type="email" 
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-white/20 uppercase tracking-widest pl-1">Signal Content</label>
                    <Textarea 
                      placeholder="How can we assist your evolution?" 
                      value={contactForm.message}
                      onChange={(e) => setContactForm({...contactForm, message: e.target.value})}
                      className="bg-white/[0.02] border-white/5 rounded-xl min-h-[150px] text-sm resize-none"
                      required
                    />
                  </div>
          ...
          <div className="lg:col-span-5 space-y-8">
              ...
              <CardHeader className="p-10 border-b border-rose-500/10">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-rose-400/40 text-xs uppercase tracking-widest font-bold">Report an interface issue</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="p-10">
                <form onSubmit={handleIssueSubmit} className="space-y-8">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-white/20 uppercase tracking-widest pl-1">Issue Type</label>
                    <Select onValueChange={(val) => setIssueForm({...issueForm, type: val})}>
                      <SelectTrigger className="h-14 bg-black/40 border-white/5 rounded-2xl">
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-white/20 uppercase tracking-widest pl-1">Description</label>
                    <Textarea 
                      placeholder="Describe the malfunction in detail..." 
                      value={issueForm.description}
                      onChange={(e) => setIssueForm({...issueForm, description: e.target.value})}
                      className="bg-black/40 border-white/5 rounded-2xl min-h-[120px] text-sm resize-none"
                      required
                    />
                  </div>
            ...
            <div className="p-8 bg-cyan-500/5 border border-cyan-500/10 rounded-[32px] flex items-center gap-6">
               ...
               <div>
                 ...
                 <p className="text-[10px] text-white/40 font-bold uppercase tracking-widest">All neural nodes online</p>
               </div>
            </div>
          </div>
        </div>
