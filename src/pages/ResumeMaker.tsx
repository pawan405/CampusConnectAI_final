...
export default function ResumeMaker() {
  const [personalInfo, setPersonalInfo] = useState({
    ...
    summary: "High-performance full-stack engineer specializing in neural architectures and AI-driven interfaces.",
  });

  const [education, setEducation] = useState([
    { id: 1, school: "Neo University", degree: "B.S. Computer Science", year: "2024" },
  ]);

  const [experience, setExperience] = useState([
    { id: 1, company: "TechNova AI", role: "Software Architect", duration: "2024 - Present", desc: "Leading the development of secure, decentralized communication protocols." },
  ]);

  const [projects, setProjects] = useState([
    { id: 1, name: "Silent Scream", tech: "React, Gemini AI", desc: "Anonymous reporting tool with neural voice scrubbing." },
  ]);
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 lg:py-20 relative z-10">
        <header className="mb-16 space-y-4">
          ...
          <h1 className="text-6xl lg:text-8xl font-black tracking-tighter uppercase">
            Resume <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-500">Maker.</span>
          </h1>
        </header>
          ...
          <div className="space-y-8">
            <Card className="bg-black/40 backdrop-blur-3xl border-white/10 rounded-[32px] overflow-hidden">
              <CardHeader className="p-8 border-b border-white/5">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-white/40 text-xs">Core identification data</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="p-8 space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input 
                      ...
                      className="bg-white/[0.02] border-white/5 rounded-xl h-12 text-sm"
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-white/20 uppercase tracking-widest pl-1">Email Signal</label>
                    <Input 
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-white/20 uppercase tracking-widest pl-1">Neural Summary</label>
                  <Textarea 
                    value={personalInfo.summary} 
                    onChange={(e) => setPersonalInfo({...personalInfo, summary: e.target.value})}
                    className="bg-white/[0.02] border-white/5 rounded-xl min-h-[100px] text-sm resize-none"
                  />
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-black/40 backdrop-blur-3xl border-white/10 rounded-[32px] overflow-hidden">
              <CardHeader className="p-8 border-b border-white/5 flex flex-row items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-white/40 text-xs">Educational milestones</CardDescription>
                  </div>
                </div>
                <Button 
                  onClick={() => setEducation([...education, { id: Date.now(), school: "", degree: "", year: "" }])}
                  variant="ghost" className="h-10 w-10 p-0 rounded-xl hover:bg-white/5 border border-white/5"
                >
              ...
              <CardContent className="p-8 space-y-6">
                {education.map((edu) => (
                  <div key={edu.id} className="p-6 bg-white/[0.02] border border-white/5 rounded-2xl space-y-4 relative group">
                    <button 
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <Input 
                        ...
                        className="bg-black/40 border-white/5 rounded-lg h-10 text-xs"
                      />
                      <Input 
                        placeholder="Degree" 
                        value={edu.degree}
            ...
            <Card className="bg-black/40 backdrop-blur-3xl border-white/10 rounded-[32px] overflow-hidden">
              <CardHeader className="p-8 border-b border-white/5 flex flex-row items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-white/40 text-xs">Technical competencies</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="p-8">
                <div className="flex flex-wrap gap-2">
                  {skills.map((skill, i) => (
                    <div key={i} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 group">
                      <span className="text-[11px] font-bold text-white/60">{skill}</span>
                      <button onClick={() => setSkills(skills.filter((_, idx) => idx !== i))} className="text-white/20 hover:text-rose-400">
                  ...
                  <Input 
                    ...
                    className="w-32 h-9 bg-black/40 border-white/5 rounded-lg text-xs"
                    onKeyDown={(e) => {
          ...
          <div className="lg:sticky lg:top-32 space-y-8">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Button onClick={handleReset} variant="ghost" className="text-white/40 hover:text-white gap-2">
                  <RotateCcw className="w-4 h-4" /> Reset
                </Button>
                <Button onClick={handleGenerate} disabled={isGenerating} className="bg-white/5 hover:bg-white/10 border-white/10 gap-2">
                  {isGenerating ? <Cpu className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4 text-cyan-400" />}
                  AI Optimize
                </Button>
              </div>
            </div>
            ...
            <div className="aspect-[1/1.4] w-full bg-white text-black p-12 shadow-[0_40px_100px_rgba(0,0,0,0.5)] rounded-[2px] font-serif relative overflow-hidden group">
              ...
              <div className="space-y-10">
                <header className="border-b-2 border-black pb-8 space-y-4">
                  ...
                  <div className="flex flex-wrap gap-x-6 gap-y-1 text-sm font-medium text-black/60">
                ...
                <section className="space-y-4">
                  <h4 className="text-xs font-black uppercase tracking-[0.2em] text-black/40 border-b border-black/10 pb-2">Professional Summary</h4>
                  <p className="text-sm leading-relaxed">{personalInfo.summary}</p>
                </section>

                <section className="space-y-4">
                  <h4 className="text-xs font-black uppercase tracking-[0.2em] text-black/40 border-b border-black/10 pb-2">Technical Matrix</h4>
                  <div className="flex flex-wrap gap-x-4 gap-y-2">
                    {skills.map((s, i) => (
                      <span key={i} className="text-xs font-bold px-2 py-0.5 border border-black/10 rounded-md bg-black/5">{s}</span>
                    ))}
                  </div>
                </section>

                <section className="space-y-4">
                  <h4 className="text-xs font-black uppercase tracking-[0.2em] text-black/40 border-b border-black/10 pb-2">Experience History</h4>
                  {experience.map(exp => (
                    <div key={exp.id} className="space-y-1">
                      ...
                      <p className="text-xs text-black/70 leading-relaxed">{exp.desc}</p>
                    </div>
                  ))}
                </section>

                <section className="space-y-4">
                  <h4 className="text-xs font-black uppercase tracking-[0.2em] text-black/40 border-b border-black/10 pb-2">Academic Record</h4>
                  {education.map(edu => (
