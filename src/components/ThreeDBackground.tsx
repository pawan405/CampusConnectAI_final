...
function FloatingShape({ position, color, type, speed = 1, size = 1 }: any) {
  ...
  const content = useMemo(() => {
    if (type === "sphere") {
      return c(
        Sphere,
        { ref: mesh, args: [size, 64, 64], position },
    ...
    if (type === "torus") {
      return c(
        "mesh",
        { ref: mesh, position, rotation: [Math.PI / 2, 0, 0] },
    ...
    if (type === "knot") {
      return c(
        TorusKnot,
        { ref: mesh, args: [size, 0.3 * size, 128, 32], position },
  ...
  return c(
    Float,
    { speed: 2 * speed, rotationIntensity: 2, floatIntensity: 2 },
    content
  );
}
